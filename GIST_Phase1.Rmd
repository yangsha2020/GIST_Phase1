...
sunan.date<-read_excel("C:\\Users\\ShaYang\\OneDrive - Stat4ward\\Documents\\Deciphera\\Phase I\\GIST\\ctDNA Patient selection summary chart.xlsx",sheet = "summary", skip = 2)
...
PD.date<-adtte%>%filter(PARAM=="Progression Free Survival (weeks)")%>%select(SUBJID, ARM,AVALC, ADT,CNSR)
```
...
sunan.date<-sunan.date%>%
  ...
  separate(PD1, into= c("PD1.sample", "PD1.date"), sep="-")%>%
  separate(PD2, into= c("PD2.sample", "PD2.date"), sep="-")%>%
  select(-c(C1D1,C3D1,C7D1,C19D1, EOT))


preIPDE.not.matching<-sunan.date%>%left_join(PreIPDE.sample, by = c("Patient_ID"="Patient_ID"))%>%select(Patient_ID, IPDE.date,IPDESTDT, preIPDE.sample,preIPDE.date, Visit_name, Bloodcoll_date)%>%
  mutate(check=case_when(
    IPDE.date==IPDESTDT&IPDE.date==Bloodcoll_date ~"Matching",
    preIPDE.sample==Visit_name~"Not Matching",
    TRUE~"not OK"))%>%
  filter(!is.na(Visit_name)&!is.na(preIPDE.sample))

#write.csv(preIPDE.not.matching, "not matching sample selections.csv")

# manual checking 
# ctDNA%>%filter(Patient_ID=="02-004")%>%select(Patient_ID, Visit_name, Somatic_status, Bloodcoll_date, Alt.NoAlt)%>%view()
